...
if __name__ == "__main__":
    wine_file_path = 'winequality-red.csv'
    R_wine = load_wine_quality_dataset(wine_file_path)
    R_wine = R_wine / np.max(R_wine)
    R_wine_normalized, _ = normalize_matrix(R_wine)

    K = 10
    alpha = 0.001
    lambda_ = 0.5
    epochs = 20
    avg_rmse_wine = cross_validate(R_wine_normalized, K, alpha, lambda_, epochs)

    print(f"Average RMSE for Wine Quality Dataset: {avg_rmse_wine:.4f}")
